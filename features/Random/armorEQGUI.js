...
function drawtheitem(ctx, item, drawX, drawY, itemScale, renderSize) {
    ...
    // Draw the item. 
    // In 1.21, item.draw(x, y, scale) is standard.
    // If it's drifting, it might be due to the scale being applied twice.
    item.draw(drawX, drawY, itemScale);
}

// 4. Define Triggers (MUST be before registration logic)
...
registerOverlay("InvGUI", { text: () => "adsadasd", align: "center", colors: false, h: 16 * 4 + 2.5 * 2, setting: () => c.invGUI });
